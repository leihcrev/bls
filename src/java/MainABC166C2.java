public final class MainABC166C2 {
  private static void execute(final IO io) throws Exception {
    ...
    for (int i = 0; i < m; i++) {
      ...
      height[0] = h[group[p[0]]];
      height[1] = h[group[p[1]]];
      System.err.println("H_" + (p[0] + 1) + "=" + height[0] + ", H_" + (p[1] + 1) + "=" + height[1]);
      if (height[0] < height[1]) {
        result = integrate(group, group[p[0]], group[p[1]], result);
      } else if (height[1] < height[0]) {
        result = integrate(group, group[p[1]], group[p[0]], result);
      }
    }
    io.println(result);
  }

  private static int integrate(int[] group, int a, int b, int result) {
    for (int i = 0; i < group.length; i++) {
      if (group[i] == a) {
        if (group[i] == i) {
          System.err.println("" + i + " is out.");
          result--;
        }
        group[i] = b;
        System.err.println("H_" + (i + 1) + " -> H_" + (b + 1));
      }
    }
    return result;
  }
